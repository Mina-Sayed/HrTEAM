...
export const getAllOverTimeByShift = async (
  ...
  if (user.role === Roles.EMPLOYEE) {
    ovts = await Overtime.find({
      shift: req.params.shift,
      users: { $elemMatch: { $in: user._id } },
    }).populate('users')
  } else {

    ovts = await Overtime.find({ shift: req.params.shift }).populate(
      'users shift',
    )
  }
  console.log(ovts)
...
export const assginUser = async (req: Request, res: Response) => {
  ...
  let user: any
  if (userId) user = await User.find({ _id: userId }).populate('shift')
...
export const unassignUser = async (req: Request, res: Response) => {
  ...
  const overtime = await Overtime.findByIdAndUpdate(
    {_id: req.params.id , shift: req.params.shift },
...
export const deleteOvertimeById = async (req: Request, res: Response) => {
  ...
  await Overtime.deleteOne({ shift: req.params.shift, _id: req.params.id })
  res.send({
    success: true,
