...
export const authBreakOver = (type: string, mode?: string) =>
    async function (req: AuthenticatedReq, res: Response, next: NextFunction)
    {
        ...
        if (type === "break" || type === "overtime") {
            ...
            const overtime: any = id && (await Overtime.findOne({ _id: id }));
            const breaks: any = id && (await Break.findOne({ _id: id }));
            console.log(req.body.shift);

            if ( !breaks && !overtime && id) {
                return res.status(400).send({ error_en: "Invalid OverTime Or Break!!" });
            }
            const shiftValid: any = overtime ? await Shift.findOne({
                _id: shift ? shift : req.body.shift ? req.body.shift : overtime.shift,
            ...
            if (req.body.userId) {
                ...
                if (
                    ...
                    return res.status(401).send({
                        error_en: `User you are trying to assign to is not found in the shift : ${ shiftValid.name } `,
                    });
                }
            }
        }
