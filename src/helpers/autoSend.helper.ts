...
import { getDateWithHoursAndMins, timeLate } from "./time.helper";
...
export const autoNotification = async () =>
{
    ...
    for (let index = 0; index < contracts.length; index++) {
        ...
        const time = getDateWithHoursAndMins(contract.endDate,
            new Date(Date.now()));
        const TimeStay = getDateWithHoursAndMins(
            contract.endStay,
            new Date(Date.now()),
        );

        if (time.days <= 15 && time.days > 13) {
            const getNotifications = await Notification.findOne({
            ...
            !getNotifications && (await Notification.insertMany(admins));
